...
class Shop:
    ...
    async def checkaction(self, mouse, towers, screen, enemies, path_surfaces, cash, sniper_number, slow_tower_id):
        ...
        for i in range(len(self.slots)):
            ...
            if self.slots[i].surface.get_rect(topleft = (self.slots[i].loc[0], self.slots[i].loc[1])).collidepoint(mouse[0],mouse[1]):
                ...
                coords = await self.slots[i].addtower(screen, enemies, towers)
                if(coords == [-2, -2]):
                    return "notexit"
                if(coords == [-1, -1]):
                    return "exit"
